...
class GoogleSheetsClient:
    ...
    def validate_sheet_structure(self) -> Dict[str, Any]:
        ...
        try:
            ...
            header_info = self._detect_header_columns()
            result['header_info'] = {
                'date_col': header_info.get('date_letter', 'None'),
            ...
            # 헤더 검증 오류 추가
            if header_info['errors']:
                result['errors'].extend(header_info['errors'])
                return result
            ...
            if all(col is not None for col in [header_info['date_col'], header_info['time_col'], header_info['account_col'], header_info['content_col']]):
                ...
                sample_range = f"{self.tab_name}!{start_col_letter}2:{end_col_letter}4"
                self.rate_limiter.wait_if_needed()
                
                sample_result = self.service.spreadsheets().values().get(
                    spreadsheetId=self.sheets_id,
                    range=sample_range
                ).execute()
                
                sample_values = sample_result.get('values', [])
